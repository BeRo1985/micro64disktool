...
{$ifndef fpc}type ptrint=longint;{$endif}
...
begin
 with F do begin
  ...
  name:=AnsiString(FindData.cFileName);
 end;
 result:=0;
end;
{$ENDIF}
  ...
  if Count>=12 then begin
   ...
   if Count>=256 then begin
    if Count<448 then begin
     ...
     PIntegerArray(P)[3] := J;
     repeat
      PIntegerArray(ptrint(P)-I)[0]:=J;
      PIntegerArray(ptrint(P)-I)[1]:=J;
      PIntegerArray(ptrint(P)-I)[2]:=J;
      PIntegerArray(ptrint(P)-I)[3]:=J;
      dec(I,16);
     until I<=0;
    end;
   end else begin
...
begin
 result:=0;
 for Counter:=1 to length(Value) do begin
